...
class RegistrationForm(FlaskForm):
    username = StringField('Username',
            validators=[DataRequired(), Length(min=2, max=20)], render_kw={'autocomplete': 'off'})
    email = StringField('Email',
            validators=[DataRequired(), Length(min=5, max=120)], render_kw={'autocomplete': 'off'})
...
class LoginForm(FlaskForm):
    email = StringField('Email',
                        validators=[DataRequired(), Email()], render_kw={'autocomplete': 'off'})
    password = PasswordField('Password', validators=[DataRequired()])
